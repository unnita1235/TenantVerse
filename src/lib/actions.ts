...
import {
  suggestRbacRoles,
  SuggestRbacRolesInput,
} from "@/ai/flows/team-onboarding-assistance";
import {
  generatePersonalizedUserGuide,
  PersonalizedUserGuideInput,
} from "@/ai/flows/personalized-user-guide-generation";

export async function getRbacSuggestions(
  previousState: any,
  formData: FormData,
) {
  try {
    const input: SuggestRbacRolesInput = {
      teamMemberProfiles: (formData.get("profiles") as string)
        .split("\n")
        .filter((p) => p.trim() !== ""),
...
export async function getPersonalizedGuide(
  previousState: any,
  formData: FormData,
) {
  try {
